...
public class showHeader extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet show http Header send by client</h1>");
            Enumeration enumu = request.getHeaderNames();
            while (enumu.hasMoreElements()) {
                String headerName = (String) enumu.nextElement();
                String headerValue = request.getHeader(headerName);
                out.println("<p>" + headerName);
                out.println(headerValue + "</p></pr>");
